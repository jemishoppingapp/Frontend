            ...
            <DialogPanel transition className="pointer-events-auto w-screen max-w-md transform transition duration-500 ease-in-out data-[closed]:translate-x-full">
              <div className="flex h-full flex-col overflow-y-auto bg-white shadow-xl">
                <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
                  ...
                  <div className="mt-8">
                    {items.length === 0 ? (
                      <div className="py-12 text-center"><p className="text-gray-500">Your cart is empty</p><Link to="/products" onClick={closeCart} className="mt-4 inline-block text-sm font-medium text-green-600">Continue Shopping →</Link></div>
                    ) : (
                        ...
                        {items.map((p) => (
                          <li key={p.id} className="flex py-6">
                            ...
                            <div className="ml-4 flex flex-1 flex-col">
                              <div><div className="flex justify-between text-base font-medium text-gray-900"><h3><Link to={p.href} onClick={closeCart}>{p.name}</Link></h3><p className="ml-4">{p.price}</p></div><p className="mt-0.5 text-xs text-gray-500">Sold by: {p.seller}</p>{p.color && <p className="mt-1 text-sm text-gray-500">{p.color}</p>}</div>
                              <div className="flex flex-1 items-end justify-between text-sm">
                                ...
                                <button onClick={() => removeItem(p.id)} className="font-medium text-green-600 hover:text-green-500">Remove</button>
                              </div>
                            </div>
                          </li>
                        ))}
                ...
                {items.length > 0 && (
                  <div className="border-t border-gray-200 px-4 py-6 sm:px-6">
                    ...
                    <p className="mt-0.5 text-sm text-gray-500">Pickup fee calculated at checkout.</p>
                    <div className="mt-6"><button onClick={handleCheckout} className="flex w-full items-center justify-center rounded-md bg-gray-900 px-6 py-3 text-base font-medium text-white hover:bg-gray-800">Checkout</button></div>
                    <div className="mt-6 flex justify-center text-sm text-gray-500"><button onClick={closeCart} className="font-medium text-green-600">Continue Shopping →</button></div>
                  </div>
                )}
              </div>
            </DialogPanel>
